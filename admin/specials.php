...
?>
    <div class="card mb-3">
      <div class="card-header">
        <h3><i class="fa fa-tag"></i> <?php echo HEADING_TITLE; ?>
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td class="smallText" align="right" valign="top"><br /><?php echo tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary', null, 'secondary') . ' ' . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('specials.php', 'page=' . $_GET['page'] . (isset($_GET['sID']) ? '&sID=' . $_GET['sID'] : ''))); ?></td>
          </tr>
        </table></td>
      </form></tr>
      </tbody>         
...
?>
      ...
      <thead>
        <tr>
          ...
          <th class="text-right" scope="col"><?php echo TABLE_HEADING_PRODUCTS_PRICE; ?></th>
...
<?php
      if ($specials['status'] == '1') {
        echo '<i class="fa fa-dot-circle-o" aria-hidden="true"></i>' . '&nbsp;&nbsp;<a href="' . tep_href_link('specials.php', 'action=setflag&flag=0&id=' . $specials['specials_id']) . '">' . '<i class="fa fa-circle-o" aria-hidden="true"></i>' . '</a>';
      } else {
        echo '<a href="' . tep_href_link('specials.php', 'action=setflag&flag=1&id=' . $specials['specials_id']) . '">' . '<i class="fa fa-circle-o" aria-hidden="true"></i>' . '</a>&nbsp;&nbsp;' . '<i class="fa fa-dot-circle-o" aria-hidden="true"></i>';
      }
?></td>
                <td class="dataTableContent" align="right"><?php if (isset($sInfo) && is_object($sInfo) && ($specials['specials_id'] == $sInfo->specials_id)) { echo '<i class="fa fa-chevron-circle-right" aria-hidden="true"></i>'; } else { echo '<a href="' . tep_href_link('specials.php', 'page=' . $_GET['page'] . '&sID=' . $specials['specials_id']) . '">' . '<i class="fa fa-info-circle" aria-hidden="true"></i>' . '</a>'; } ?>&nbsp;</td>
      </tr>
<?php
    }
?>
...
<?php
  ...
  switch ($action) {
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary', null, 'secondary') . ' ' . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('specials.php', 'page=' . $_GET['page'] . '&sID=' . $sInfo->specials_id)));
      break;
    default:
      if (is_object($sInfo)) {
        $heading[] = array('text' => '<strong>' . $sInfo->products_name . '</strong>');
        $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('specials.php', 'page=' . $_GET['page'] . '&sID=' . $sInfo->specials_id . '&action=edit'), null, null, 'secondary') . ' ' . tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('specials.php', 'page=' . $_GET['page'] . '&sID=' . $sInfo->specials_id . '&action=delete')));
...
<script>
  $(document).ready(function () {
    $('input[name="expdate"]').daterangepicker({
    ...
    $('input[name="expdate"]').on('apply.daterangepicker', function(ev, picker) {
        $(this).val(picker.startDate.format('YYYY-MM-DD'));
    });
    $('input[name="expdate"]').on('cancel.daterangepicker', function(ev, picker) {
        $(this).val('');
    });
