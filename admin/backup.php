  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'backupnow':
        ...
        $schema = '# FieldMitr, Open Source E-Commerce Solutions' . "\n" .
                  '# http://www.oscommerce.com' . "\n" .
                  '#' . "\n" .
  ...
  <div class="col-sm-9">
        <div class="card mb-3">
                  <div class="card-header">
                    <h3><i class="fa fa-database"></i> <?php echo HEADING_TITLE; ?>
                  ...
                  <div class="card-body">  
                    <div class="table-responsive">
                      ...
                      <thead>
                        <tr>
                          <th scope="col"><?php echo TABLE_HEADING_TITLE; ?></th>
                          <th scope="col"><?php echo TABLE_HEADING_FILE_DATE; ?></th>
                          <th scope="col"><?php echo TABLE_HEADING_FILE_SIZE; ?></th>
...
?>
                <td class="dataTableContent" onclick="document.location.href='<?php echo tep_href_link('backup.php', $onclick_link); ?>'"><?php echo '<a href="' . tep_href_link('backup.php', 'action=download&file=' . $entry) . '">' . '<i class="fa fa-download" aria-hidden="true"></i>' . '</a>&nbsp;' . $entry; ?></td>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($buInfo) && is_object($buInfo) && ($entry == $buInfo->file)) { echo '<i class="fa fa-chevron-circle-right" aria-hidden="true"></i>'; } else { echo '<a href="' . tep_href_link('backup.php', 'file=' . $entry) . '">' . '<i class="fa fa-info-circle" aria-hidden="true"></i>' . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
    $dir->close();
...
?>
              <tr>
                ...
                <td align="right" class="smallText"><?php if ( ($action != 'backup') && (isset($dir)) ) echo tep_draw_button(IMAGE_BACKUP, 'copy', tep_href_link('backup.php', 'action=backup'), null, null, 'secondary'); if ( ($action != 'restorelocal') && isset($dir) ) echo ' ' . tep_draw_button(IMAGE_RESTORE, 'arrowrefresh-1-w', tep_href_link('backup.php', 'action=restorelocal')); ?></td>
              </tr>
<?php
...
<?php
  ...
  switch ($action) {
    case 'backup':
      ...
      $contents[] = array('text' => '<br />' . tep_draw_bs_radio_field('compress', 'no', true, '', TEXT_INFO_USE_NO_COMPRESSION, ''));
      ...
      if ($dir_ok == true) {
        $contents[] = array('text' => '<br />' . tep_draw_bs_checkbox_field('download', 'yes', '', '', TEXT_INFO_DOWNLOAD_ONLY, '')  . '<br />*' . TEXT_INFO_BEST_THROUGH_HTTPS);
      } else {
        $contents[] = array('text' => '<br />' . tep_draw_radio_field('download', 'yes', true) . ' ' . TEXT_INFO_DOWNLOAD_ONLY . '*<br /><br />*' . TEXT_INFO_BEST_THROUGH_HTTPS);
      }

      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_BACKUP, 'copy', null, 'primary', null, 'secondary') . ' ' . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('backup.php')));
      break;
    case 'restore':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_RESTORE, 'arrowrefresh-1-w', tep_href_link('backup.php', 'file=' . $buInfo->file . '&action=restorenow'), 'primary', null, 'secondary') . ' ' . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('backup.php', 'file=' . $buInfo->file)));
      break;
    case 'restorelocal':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_RESTORE, 'arrowrefresh-1-w', null, 'primary', null, 'secondary') . ' ' . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('backup.php')));
      break;
    case 'delete':
      $heading[] = array('text' => '<strong>' . $buInfo->date . '</strong>');
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary', null, 'secondary') . ' ' . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('backup.php', 'file=' . $buInfo->file)));
      break;
    default:
      if (isset($buInfo) && is_object($buInfo)) {
        $heading[] = array('text' => '<strong>' . $buInfo->date . '</strong>');

        $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_RESTORE, 'arrowrefresh-1-w', tep_href_link('backup.php', 'file=' . $buInfo->file . '&action=restore'), null, null, 'secondary') . ' ' . tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('backup.php', 'file=' . $buInfo->file . '&action=delete')));
