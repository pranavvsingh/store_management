  ...
  switch ( $action ) {
    ...
    case 'repair':
     ...
     /*  $table_headers = array(TABLE_HEADING_TABLE,
      ...
      $table_headers = array(TABLE_HEADING_TABLE,
                             TABLE_HEADING_MSG_TYPE,
                             TABLE_HEADING_MSG,
                             '<input type="checkbox" name="masterblaster">');
    ...
    case 'utf8':
      ...
      /*   $table_headers = array(TABLE_HEADING_TABLE,
                               ...
                               tep_draw_checkbox_field('masterblaster')); */
        $table_headers = array(TABLE_HEADING_TABLE,
                               TABLE_HEADING_MSG,'<input type="checkbox" name="masterblaster">');
...
// mysqli_query() is directly called as tep_db_query() dies when an error occurs
        ...
        if ( !isset($_POST['dryrun']) ) {
         ...
         /*  $table_data[] = array(tep_output_string_protected($table),
                                tep_output_string_protected($result),'<input id="id[]" type="checkbox" name="masterblaster" checked>'); */
  ...
  <div class="col-sm-12">
        <div class="card mb-3">
          ...
          <div class="row">
            ...
            <div class="col-sm-6">
            <h3><i class="fa fa-database"></i> <?php echo HEADING_TITLE; ?></h3>
            </div>
...
?>
  ...
  <?php echo '<span class="runUtf8" style="display: none;">' . sprintf(ACTION_UTF8_DRY_RUN, tep_draw_checkbox_field('dryrun')) . '</span>' . tep_draw_pull_down_menu('action', $actions, '', 'id="sqlActionsMenu"') . '<span class="runUtf8" style="display: none;">&nbsp;' . tep_draw_pull_down_menu('from_charset', $mysql_charsets) . '</span>';
...
$(document).ready(function () {
  if ( $('input[type="checkbox"][name="masterblaster"]').length > 0 ) {
    $('input[type="checkbox"][name="masterblaster"]').click(function() {
      $('input[type="checkbox"][name="id[]"]').prop('checked', $('input[type="checkbox"][name="masterblaster"]').prop('checked'));
    });
  }
