      ...
      <div class="col-sm-9">
            <div class="card mb-3">
                      ...
                      <span style="float:right"><?php
                          if (empty($action)) {
                        ?>
                          <?php echo tep_draw_button(IMAGE_UPDATE_CURRENCIES, 'refresh', tep_href_link('currencies.php', 'page=' . $_GET['page'] . '&cID=' . $cInfo->currencies_id . '&action=update'), null, null, 'secondary'); ?> <?php echo tep_draw_button(IMAGE_NEW_CURRENCY, 'plus', tep_href_link('currencies.php', 'page=' . $_GET['page'] . '&cID=' . $cInfo->currencies_id . '&action=new')); ?>
            ...
            <thead>
              <tr>
                <th scope="col"><?php echo TABLE_HEADING_CURRENCY_NAME; ?></th>
                <th scope="col"><?php echo TABLE_HEADING_CURRENCY_CODES; ?></th>
                <th scope="col"><?php echo TABLE_HEADING_CURRENCY_VALUE; ?></th>
...
?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($cInfo) && is_object($cInfo) && ($currency['currencies_id'] == $cInfo->currencies_id) ) { echo '<i class="fa fa-chevron-circle-right"></i>'; } else { echo '<a href="' . tep_href_link('currencies.php', 'page=' . $_GET['page'] . '&cID=' . $currency['currencies_id']) . '">' . '<i class="fa fa-info-circle" aria-hidden="true"></i>' . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
  }
?>
...
<?php
  ...
  switch ($action) {
    case 'new':
      ...
      $contents[] = array('text' => '<br />' . tep_draw_bs_checkbox_field('default','','','',TEXT_INFO_SET_AS_DEFAULT,''));
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary', null, 'secondary') . ' ' .  tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('currencies.php', 'page=' . $_GET['page'] . '&cID=' . $_GET['cID'])));
      break;
    case 'edit':
      ...
      if (DEFAULT_CURRENCY != $cInfo->code) $contents[] = array('text' => '<br />' . tep_draw_bs_checkbox_field('default','','','',TEXT_INFO_SET_AS_DEFAULT,''));
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary', null, 'secondary') . ' ' .  tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('currencies.php', 'page=' . $_GET['page'] . '&cID=' . $cInfo->currencies_id)));
      break;
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . (($remove_currency) ? tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('currencies.php', 'page=' . $_GET['page'] . '&cID=' . $cInfo->currencies_id . '&action=deleteconfirm'), 'primary', null, 'secondary') : '') . ' ' . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('currencies.php', 'page=' . $_GET['page'] . '&cID=' . $cInfo->currencies_id)));
      break;
    default:
      if (is_object($cInfo)) {
        $heading[] = array('text' => '<strong>' . $cInfo->title . '</strong>');

        $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('currencies.php', 'page=' . $_GET['page'] . '&cID=' . $cInfo->currencies_id . '&action=edit'), null, null, 'secondary') . ' ' . tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('currencies.php', 'page=' . $_GET['page'] . '&cID=' . $cInfo->currencies_id . '&action=delete')));
