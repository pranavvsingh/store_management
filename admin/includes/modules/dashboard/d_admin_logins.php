    ...
    function getOutput() {
      ...
      <div class="card mb-3">
                <div class="card-header">
                  <h6><i class="fa fa-lock"></i> ' . MODULE_ADMIN_DASHBOARD_ADMIN_LOGINS_TITLE . '
                ...
                <thead>
                  <tr>
                  <th scope="col"></th>
                  <th scope="col">' . MODULE_ADMIN_DASHBOARD_ADMIN_LOGINS_TITLE . '</th>
                    <th scope="col">' . MODULE_ADMIN_DASHBOARD_ADMIN_LOGINS_DATE . '</th>
      ...
      $logins_query = tep_db_query("select id, user_name, success, date_added from " . TABLE_ACTION_RECORDER . " where module = 'ar_admin_login' order by date_added desc limit 5");
      while ($logins = tep_db_fetch_array($logins_query)) {
        $output .= '  <tr class="dataTableRow" onmouseover="rowOverEffect(this);" onmouseout="rowOutEffect(this);">' .
                   '    <td class="dataTableContent" align="center">' . (($logins['success'] == '1') ? '<i class="fa fa-check" aria-hidden="true"></i>' : '<i class="fa fa-close" aria-hidden="true"></i>') . '</td>' .
