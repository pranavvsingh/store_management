  ...
  <div class="col-sm-12">
    <div class="card mb-3">
      <div class="card-header">
        <h3><i class="fa fa-shopping-basket"></i> <?php echo HEADING_TITLE . ': #' . (int)$oID . ' (' . $order->info['total'] . ')'; ?>
        <span style="float:right">
        <?php echo tep_draw_button(IMAGE_ORDERS_INVOICE, 'document', tep_href_link('invoice.php', 'oID=' . $_GET['oID']), null, array('newwindow' => true), 'secondary') . ' ' . tep_draw_button(IMAGE_ORDERS_PACKINGSLIP, 'document', tep_href_link('packingslip.php', 'oID=' . $_GET['oID']), null, array('newwindow' => true), 'primary') . ' ' . tep_draw_button(IMAGE_BACK, 'triangle-1-w', tep_href_link('orders.php', tep_get_all_get_params(array('action')))); ?>
      </span></h3>
      </div>
      <div class="card-body">
<div class="row jumbotron">
  <div class="col-sm-3">
            <h6><i class="fa fa-map-marker" aria-hidden="true"></i> <?php echo ENTRY_CUSTOMER; ?></h6>
            <p><?php echo tep_address_format($order->customer['format_id'], $order->customer, 1, '', '<br />'); ?></p>
            <p><?php echo '<i class="fa fa-phone" aria-hidden="true"></i>'.' '.$order->customer['telephone'] . '<br />' . '<i class="fa fa-envelope-o" aria-hidden="true"></i> <a href="mailto:' . $order->customer['email_address'] . '"><u>' . $order->customer['email_address'] . '</u></a>'; ?></p>
  </div>
  <div class="col-sm-3">
            <h6><i class="fa fa-truck" aria-hidden="true"></i> <?php echo ENTRY_SHIPPING_ADDRESS; ?></h6>
            <p><?php echo tep_address_format($order->delivery['format_id'], $order->delivery, 1, '', '<br />'); ?></p>
  </div>
  <div class="col-sm-3">
            <h6><i class="fa fa-address-card-o" aria-hidden="true"></i> <?php echo ENTRY_BILLING_ADDRESS; ?></h6>
            <p><?php echo tep_address_format($order->billing['format_id'], $order->billing, 1, '', '<br />'); ?></p>
  </div>
  <div class="col-sm-3">
            <h6><i class="fa fa-credit-card" aria-hidden="true"></i> <?php echo ENTRY_PAYMENT_METHOD; ?></h6>
...
<div class="row">
  <div class="col-sm-12">
    <div class="table-responsive">
      ...
      <thead>
        <tr>
          <th colspan="2" scope="col"><?php echo TABLE_HEADING_PRODUCTS; ?></th>
          <th scope="col"><?php echo TABLE_HEADING_PRODUCTS_MODEL; ?></th>
          <th class="text-right" scope="col"><?php echo TABLE_HEADING_TAX; ?></th>
          <th class="text-right" scope="col"><?php echo TABLE_HEADING_PRICE_EXCLUDING_TAX; ?></th>
          <th class="text-right" scope="col"><?php echo TABLE_HEADING_PRICE_INCLUDING_TAX; ?></th>
          <th class="text-right" scope="col"><?php echo TABLE_HEADING_TOTAL_EXCLUDING_TAX; ?></th>
          <th class="text-right" scope="col"><?php echo TABLE_HEADING_TOTAL_INCLUDING_TAX; ?></th>
        </tr>
      </thead>
      <tbody>
<?php
...
<div class="form-group">
    <label><?php echo ENTRY_ADD_COMMENT; ?></label>
    <?php echo tep_draw_textarea_field('comments', 'soft', '60', '3', null, 'style="width: 100%;"'); ?>
    </div>
<div class="form-check form-check-inline">
  <?php echo tep_draw_checkbox_field('notify', '', true); ?>
  <label class="form-check-label" for="inlineCheckbox1"><?php echo ENTRY_NOTIFY_CUSTOMER; ?></label>
</div>
<div class="form-check form-check-inline">
  <?php echo tep_draw_checkbox_field('notify_comments', '', true); ?>
  <label class="form-check-label" for="inlineCheckbox2"><?php echo ENTRY_NOTIFY_COMMENTS; ?></label>
</div>
<div class="form-check form-check-inline">
  ...
  <div class="table-responsive">
          ...
          <thead>
            <tr>
              ...
              <th scope="col" class="text-center"><?php echo TABLE_HEADING_COMMENTS; ?></th>
              <th scope="col" class="text-right"><?php echo TABLE_HEADING_CUSTOMER_NOTIFIED; ?></th>
            </tr>
          </thead>
          <tbody>
<?php
  ...
  <div class="col-sm-9">
    <div class="card mb-3">
      <div class="card-header">
        <div class="row">
          <div class="col-sm-4">
          <h3><i class="fa fa-shopping-basket"></i> <?php echo HEADING_TITLE; ?></h3>
          </div>
          <div class="col-sm-4">
        ...
        <div class="table-responsive">
          ...
          <thead>
            <tr>
              ...
              <th scope="col"><?php echo TABLE_HEADING_ORDER_TOTAL; ?></th>
              <th scope="col"><?php echo TABLE_HEADING_DATE_PURCHASED; ?></th>
...
?>
                <td class="dataTableContent"><?php echo '<a href="' . tep_href_link('orders.php', tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $orders['orders_id'] . '&action=edit') . '">' . '<i class="fa fa-search" aria-hidden="true"></i>' . '</a>&nbsp;' . $orders['customers_name']; ?></td>
...
<?php
  ...
  switch ($action) {
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary', null, 'secondary') . ' ' . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('orders.php', tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id)));
      break;
    default:
      if (isset($oInfo) && is_object($oInfo)) {
        $heading[] = array('text' => '<strong>[' . $oInfo->orders_id . ']&nbsp;&nbsp;' . tep_datetime_short($oInfo->date_purchased) . '</strong>');
        $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('orders.php', tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=edit'), null, null, 'secondary') . ' ' . tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('orders.php', tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=delete'), null, null, 'danger'));
        $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_ORDERS_INVOICE, 'document', tep_href_link('invoice.php', 'oID=' . $oInfo->orders_id), null, array('newwindow' => true), 'primary') . ' ' . tep_draw_button(IMAGE_ORDERS_PACKINGSLIP, 'document', tep_href_link('packingslip.php', 'oID=' . $oInfo->orders_id), null, array('newwindow' => true)));
