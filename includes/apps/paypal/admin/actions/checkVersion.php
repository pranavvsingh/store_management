...
*/
  ...
  if ( class_exists('ZipArchive') && function_exists('json_encode') && function_exists('openssl_verify') ) {
    ...
    $ppUpdateReleasesResponse = @json_decode($OSCOM_PayPal->makeApiCall('http://apps.FieldMitr.com/index.php?RPC&GetUpdates&paypal&app&2_3&' . str_replace('.', '_', number_format($OSCOM_PayPal->getVersion(), 3))), true);
    ...
    $ppUpdateReleasesResponse = $OSCOM_PayPal->makeApiCall('http://apps.FieldMitr.com/index.php?RPC&GetUpdates&paypal&app&2_3&' . str_replace('.', '_', number_format($OSCOM_PayPal->getVersion(), 3)) . '&format=simple');
