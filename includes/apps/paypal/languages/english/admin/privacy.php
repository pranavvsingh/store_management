...
<p>The PayPal App for FieldMitr Online Merchant allows store owners to automatically setup and configure the App with their PayPal API credentials without the need to enter them manually. This is performed securely by granting FieldMitr access to retrieve the API credentials from the store owners PayPal account.</p>

<p>Granting FieldMitr access allows the following limited information to be retrieved from the store owners PayPal account:</p>
...
<p>The process is started by using the "Retrieve Live Credentials" and "Retrieve Sandbox Credentials" buttons displayed on the PayPal App start and credentials management pages. The store owner is securely taken to PayPal's website where they are asked to grant FieldMitr access to retrieve the API credentials, and are then redirected back to their online store to continue configuration of the PayPal App. This is performed with the following steps:</p>

<ol>
  <li>The store owner clicks on "Retrieve Live Credentials" or "Retrieve Sandbox Credentials" and is securely taken to an initialization page on the FieldMitr website that registers the request and immediately redirects the store owner to an on-boarding page on the PayPal website. FieldMitr registers the following information in the request:
    <ul>
  ...
  <li>PayPal asks the store owner to grant FieldMitr permission to retrieve their API credentials.</li>
  <li>PayPal redirects the store owner back to the initialization page on the FieldMitr website.</li>
  <li>FieldMitr securely retrieves and stores the following information from PayPal:
    <ul>
      <li>API Username</li>
      <li>API Password</li>
      <li>API Signature</li>
  ...
  <li>The PayPal App performs a secure HTTPS call to the FieldMitr website to retrieve the API credentials.</li>
  <li>The FieldMitr website authenticates the secure HTTPS call, sends the API credentials, and locally discards the API credentials and PayPal App URL stored in steps 1 and 5.</li>
  <li>The PayPal App configures itself with the API credentials.</li>
</ol>

<div class="pp-panel pp-panel-warning">
  <p>The API Credentials retrieved from the store owners PayPal account are only used to configure the PayPal App. FieldMitr temporarily stores the API Credentials as described in this privacy policy, and discards the API Credentials as soon as the process is over. A back-end script is also run to discard any stored information for processes that have not finalized.</p>
</div>

<div class="pp-panel pp-panel-info">
  <p>FieldMitr has worked closely with PayPal to ensure the PayPal App follows strict privacy and security policies.</p>
</div>
...
<p>The PayPal App for FieldMitr Online Merchant automatically checks the FieldMitr website for updates that are available to the App. This check is performed once every 24 hours and if an update is available, a notification is shown to allow the App to download and apply the update.</p>
