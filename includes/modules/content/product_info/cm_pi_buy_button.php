...
*/
  ...
  class cm_pi_buy_button {
    ...
    function __construct() {
      ...
      $this->title = MODULE_CONTENT_PI_BUY_TITLE;
      $this->description = MODULE_CONTENT_PI_BUY_DESCRIPTION;
      $this->description .= '<div class="secWarning">' . MODULE_CONTENT_BOOTSTRAP_ROW_DESCRIPTION . '</div>';

      if ( defined('MODULE_CONTENT_PI_BUY_STATUS') ) {
        $this->sort_order = MODULE_CONTENT_PI_BUY_SORT_ORDER;
        $this->enabled = (MODULE_CONTENT_PI_BUY_STATUS == 'True');
      }
    }
    ...
    function check() {
      return defined('MODULE_CONTENT_PI_BUY_STATUS');
    }
    ...
    function install() {
      ...
      tep_db_query("insert into configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_CONTENT_PI_BUY_SORT_ORDER', '100', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
    }
    ...
    function keys() {
      return array('MODULE_CONTENT_PI_BUY_STATUS', 'MODULE_CONTENT_PI_BUY_CONTENT_WIDTH', 'MODULE_CONTENT_PI_BUY_SORT_ORDER');
    }
  }
