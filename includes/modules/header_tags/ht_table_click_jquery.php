...
*/
  ...
  class ht_table_click_jquery {
    ...
    function install() {
      ...
      tep_db_query("insert into configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_HEADER_TAGS_TABLE_CLICK_JQUERY_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
    }
  ...
  function ht_table_click_jquery_edit_pages($values, $key) {
    ...
    foreach ($files_array as $file) {
      $output .= tep_draw_bs_checkbox_field('ht_table_click_jquery_file[]', $file, in_array($file, $values_array),'',tep_output_string($file),'');
    }
